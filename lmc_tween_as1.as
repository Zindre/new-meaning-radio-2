...
/*
18.10.2004 version 1.1.8 AS1
pauseAll() unpauseAll() now works with loaded movies (thanks to J.G.)
fixed bug in stopTween with delayed tweens(thanks to Petzo)
...
depth of controlling Movieclip is now negative /-6789/ (thanks to Ivan Dembicki) 
fix FP7 problems using _global keyword (thanks to orangechicken)
fixed growing of ints array when you use very much delayed tweens 
found and fixed FP7 problem with animtype parameter the animtype must be written in correct way ('easeOutBounce' not only 'easeoutbounce') else is used default easeOutExpo
fixed bug with bandwich profiler, if you press ctrl enter during tween the tweens was not running
*/
//
...
with(this){
	...
	intid = setInterval(function(obj){
		...
		// cleanup ints array if all is undefined
